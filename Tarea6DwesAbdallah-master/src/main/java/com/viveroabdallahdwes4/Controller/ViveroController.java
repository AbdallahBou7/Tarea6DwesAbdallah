...
public class ViveroController {
    ...
    public String listarEjemplares(Model model) {
        List<Ejemplar> ejemplares = ejemplarService.findAll();
        model.addAttribute("ejemplares", ejemplares);
        return "listarEjemplares"; 
    }
    
    @GetMapping("/ejemplares/listar")
    public String listarEj(Model model) {
        List<Ejemplar> ejemplares = ejemplarService.findAll();
        if (ejemplares == null) {
    ...
    public String nuevoEjemplarSubmit(@ModelAttribute Ejemplar ejemplar, HttpSession session) {
        ...
        return "redirect:/ejemplares/listar";
        }
 ...
 // CU6: Gestión de Mensajes 
    ...
    @GetMapping("/nuevo")
    public String mostrarFormularioMensaje(Model model) {
        ...
        if (ejemplares.isEmpty()) {
            System.out.println("No hay ejemplares en la base de datos.");
        }

        for (Ejemplar e : ejemplares) {
            if (e.getPlanta() == null) {
                System.out.println(" El ejemplar con ID " + e.getId() + " no tiene una planta asociada.");
            }
        }
